...
void WebService::init() {
    ...
    if (curl) {
		...
		//ssl private key
        curl_easy_setopt( curl, CURLOPT_SSLKEY, Settings::ias_privkey);
...
vector<pair<string, string> > WebService::parseJSONfromIAS(string json) {
    ...
    if (!parsingSuccessful) {
		cout << "Failed to parse JSON string from IAS" << endl;
