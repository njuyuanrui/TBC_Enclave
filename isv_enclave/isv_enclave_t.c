...
static sgx_status_t SGX_CDECL sgx_tmac_get_report(void* pms)
{
	...
	if (_tmp_req_pubkey != NULL && _len_req_pubkey != 0) {
		_in_req_pubkey = (uint8_t*)malloc(_len_req_pubkey);
		if (_in_req_pubkey == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memcpy(_in_req_pubkey, _tmp_req_pubkey, _len_req_pubkey);
	}
	...
	if (_tmp_req_pubkey_sig != NULL && _len_req_pubkey_sig != 0) {
		if ((_in_req_pubkey_sig = (uint8_t*)malloc(_len_req_pubkey_sig)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}
...
SGX_EXTERNC const struct {
	...
	4,
	{
		{(void*)(uintptr_t)sgx_tmac_get_report, 0},
...
sgx_status_t SGX_CDECL ocall_print_pubkey(const char* pubkey)
{
	sgx_status_t status = SGX_SUCCESS;
	size_t _len_pubkey = pubkey ? strlen(pubkey) + 1 : 0;

	ms_ocall_print_pubkey_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_ocall_print_pubkey_t);
	void *__tmp = NULL;

	ocalloc_size += (pubkey != NULL && sgx_is_within_enclave(pubkey, _len_pubkey)) ? _len_pubkey : 0;
	...
	ms = (ms_ocall_print_pubkey_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_ocall_print_pubkey_t));
